...
- **Usage-limit and rate-limit policies were destroyed and recreated when `conditions` changed, silently resetting the budget counter** — `conditions`, `periodic_reset`, and `periodic_reset_days` on `portkey_usage_limits_policy`, and `conditions` on `portkey_rate_limits_policy`, carried `RequiresReplace` plan modifiers. Terraform therefore never invoked the provider's `Update` for those attributes; it issued a `DELETE` followed by a `POST`. Because Portkey stores spend counters in `usage_limits_policy_value_key_map` keyed by policy ID, and enforcement only reads policies whose `status` is `active`, the archived policy's counters were orphaned and the replacement started from `current_usage: 0`. The visible symptom was the Analytics tab and the policy page disagreeing on spend (e.g. `$310.10` vs `$200.93` for the same user), but the real consequence was an enforcement gap: the gateway blocks on `current_usage >= credit_limit`, so a user reset to zero was granted their full budget again. Adding or removing a user from a budget — the most routine edit there is — triggered this on every apply. The Portkey API has accepted `conditions` on `PUT /policies/usage-limits/{id}` since backend v1.16.0 and never touches the counter table unless `reset_usage_for_value` is passed, so the destroy/recreate was the provider imposing a rebuild the API did not need. These attributes now update in place and the accumulated usage survives. `group_by`, `type`, and `workspace_id` still force replacement, correctly — the update endpoint does not accept them. **Upgrade note:** the first plan after upgrading will show `~ update in-place` where it previously showed `-/+ destroy and then create replacement`. No state migration is required, and policies already replaced by an earlier version cannot have their lost counters recovered.
- **`alert_threshold` could not be cleared** — `Update` skipped the attribute entirely when the planned value was null, so removing `alert_threshold` from a configuration left the previous threshold live on the server while Terraform reported success. The field is now always sent, as an explicit `null` when unset, which the API accepts as a clear. `name` is still omitted rather than nulled, because the API rejects a null name.
- **In-place policy updates risked "Provider produced inconsistent result after apply"** — `Create` preserved the planned `conditions` / `group_by` JSON after mapping the API response into state, but `Update` did not, so an update could write a differently-ordered but semantically identical JSON string into state and trip Terraform's post-apply consistency check. `Update` now preserves the planned values the same way `Create` does.

### Changed
- **`created_at` on both policy resources no longer shows as `(known after apply)` during updates** — the attribute is immutable, so it now carries `UseStateForUnknown`. This is cosmetic: it removes a spurious line from every policy update plan. `status` deliberately does not get the same treatment, since it can legitimately change out of band.

### Known limitations
- **Policy `conditions` drift is still not detected** — `Read` continues to preserve `conditions`, `group_by`, `type`, and the reset cadence from state rather than refreshing them from the API, so a change made in the Portkey UI will not appear as a plan diff. This predates the fix above and is unchanged by it; the fix only ensures that changes made *in Terraform* are applied in place instead of by rebuilding the policy.
