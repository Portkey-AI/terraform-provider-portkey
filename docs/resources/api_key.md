...
~> **Important:** Once `usage_limits` or `rate_limits` are set on an API key, they **cannot be removed via the API**. Removing them from your Terraform config will show a plan diff, but the limits will persist on the Portkey backend. To clear limits, use the Portkey UI.
