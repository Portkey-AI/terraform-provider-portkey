...
- `allow_config_override` (Boolean) Controls whether callers using this API key can override the bound `config_id` at request time. Only meaningful when `config_id` is set. Set to `false` (default) to lock callers to the bound config; set to `true` to allow per-request config overrides. Once set, clearing this field in Terraform will not remove it from the API — use the Portkey API directly to unset it.
