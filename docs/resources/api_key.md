...
Setting `allow_config_override = true` lets callers supply their own config at request time. Omitting both `config_id` and `allow_config_override` leaves the key unbound (no default config).
