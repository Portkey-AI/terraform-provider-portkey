...
Vertex AI does not use `key`. Set `ai_provider_id = "vertex-ai"` and pass `configurations` with `vertex_auth_type`, `vertex_region`, and either `vertex_service_account_json` or `vertex_project_id` depending on the auth mode. See the [Create Integration API](https://portkey.ai/docs/api-reference/admin-api/control-plane/integrations/create-integration#body-configurations-one-of-3-vertex-auth-type).
...
### Google Vertex AI (basic / ambient GCP credentials)

Use `vertex_auth_type = "basic"` when the gateway host supplies GCP credentials (ADC, GKE Workload Identity, etc.). Requires `vertex_project_id` and `vertex_region`.
...
|-------|---------------|-------------|
| `vertex_auth_type` | Always | `"basic"` or `"serviceAccount"` |
| `vertex_region` | Always | GCP region (e.g. `us-central1`) |
| `vertex_project_id` | `basic` | GCP project ID |
| `vertex_service_account_json` | `serviceAccount` | Full GCP service account key JSON object |
